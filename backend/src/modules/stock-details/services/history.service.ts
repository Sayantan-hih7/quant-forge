...
export async function stockHistory(stock: Instrument, frame: StockTimeframe) {
      ...
      if (!request) {
        request = (async () => {
          if (intraday) {
            ...
            const receipt = await DatasetReceiptModel.findOne({ instrumentId: stock._id, kind: 'daily', from: { $lte: from } }).sort({ to: -1 }).lean();
            if (!receipt?.to || receipt.to < today) await ensureMonthlyHistory(stock, receipt?.to && receipt.to > from ? receipt.to : from, today, { maxWaitMs: 30_000 });
          }
  ...
  return { instrumentId: stock._id, timeframe: frame, bars, message, refreshedAt: new Date().toISOString(), source: 'Dhan historical candles', timezone: 'Asia/Kolkata' };
}
