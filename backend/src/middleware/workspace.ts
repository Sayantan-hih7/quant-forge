...
export const startLocalSession: RequestHandler = async (req, res) => {
  ...
  if (env.NODE_ENV === 'production' || !loopback || !isAllowedFrontendOrigin(req.get('origin'))) throw new AppError(403, 'SESSION_FORBIDDEN', 'This address cannot open a local workspace session. Open the app using its configured local address.');
